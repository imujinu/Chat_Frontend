<template>
  <v-container>
      ...
      <v-col>
        <v-card>
          ...
          <div class="d-flex justify-end">
            <v-btn color="secondary" @click="showCreateRoomModal()"
              >채팅방 생성</v-btn
            >
          </div>
    ...
    <v-dialog v-model="showCreateRoomModal" max-width="500px">
      ...
      <v-card-text>
        <v-text-field labe="방제목" v-model="newRoomTitle" />
      </v-card-text>
      <v-card-actions>
...
export default {
  ...
  methods: {
    ...
    async createChatRoom() {
      await axios.post(
        `${process.env.VUE_APP_API_BASE_URL}/room/group/create?roomName=${this.newRoomTitle}`,
